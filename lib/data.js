export async function loadJson(url, errPrefix = "Failed to load JSON") {
  const res = await fetch(url, { cache: "no-store" });
  if (!res.ok) {
    throw new Error(`${errPrefix}: ${res.status} ${res.statusText}`);
  }
  return res.json();
}

export async function loadDashboardModel(url) {
  return loadJson(url, "Failed to load JSON");
}

export async function loadAssistantSummary(url) {
  return loadJson(url, "Failed to load assistant summary");
}

export async function loadAssistantPersona(url) {
  return loadJson(url, "Failed to load assistant persona");
}
