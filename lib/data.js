...
export function applyTriageToBlocks(blocks, state, now = new Date()) {
  ...
  const byId = new Map();
  for (const b of blocks) for (const it of (b.items || [])) byId.set(it.email_id, { ...it, _source_category: b.category });

  const snoozedItems = [];
  for (const [emailId, entry] of Object.entries(s.snoozed)) {
    if (isSnoozedActive(entry, now) && byId.has(emailId)) {
      snoozedItems.push({ ...byId.get(emailId), _snooze_until: entry.until, _snooze_preset: entry.preset });
    }
  }
